...
public class FollowCamBehaviour : MonoBehaviour
{
    ...
    void Update()
    {
        transform.position = new Vector3(_trackingTarget.position.x,
_trackingTarget.position.y, transform.position.z);
        ...
        if (!_isXLocked)
        {
            xNew = Mathf.Lerp(transform.position.x, xTarget,
            Time.deltaTime * _followSpeed);
        }
        float yNew = transform.position.y;
        if (!_isYLocked)
        {
            yNew = Mathf.Lerp(transform.position.y, yTarget,
            Time.deltaTime * _followSpeed);
        }
        transform.position = new Vector3(xNew, yNew,
